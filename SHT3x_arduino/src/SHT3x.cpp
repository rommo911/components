...
{
	if (SDA == 0 && SCL == 0)
		_OperationEnabled = Wire.begin();
	else
		_OperationEnabled = Wire.begin(SDA, SCL);
	SoftReset();

}

bool	SHT3x::UpdateData(int SDA)
{
	_Error = noError;
	if ((_LastUpdateMillisec == 0) || ((millis() - _LastUpdateMillisec) >= _UpdateIntervalMillisec))
	{
		...
		if (_Error == noError && res)
		{
			...
			{
				delay(10);
				ESP_LOGI("SHT3x", "Waiting for data %d", Wire.available());
			}
			if ((millis() - WaitingBeginTime) < _TimeoutMillisec)
			{
				...
				else
				{
					...
					ESP_LOGE("SHT3x", "DataCorrupted");
				}
			}
			else //Timeout
			{
				ESP_LOGE("SHT3x", "Timeout");
				_Error = Timeout;
			}
		...
		{
			ESP_LOGE("SHT3x", "Error after command send");
...
void	SHT3x::HardReset()
{
	if (_HardResetPin < 100) //MEGA2560 have only 68 pins
	{
		...
		digitalWrite(_HardResetPin, 0);
		delay(10); //Reset pin have to be LOW at least 350ns, so 1 usec is enough. I think, there is no need for micros() for 1 usec.
		digitalWrite(_HardResetPin, 1);
	}
}
...
bool 	SHT3x::SendCommand(uint8_t MSB, uint8_t LSB, int SDA)
{
	...
	{
		if (SDA == 0) Wire.begin();
		else _OperationEnabled = Wire1.begin();
	}
