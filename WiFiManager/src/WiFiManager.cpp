...
WiFiManagerParameter::WiFiManagerParameter(const String& custom)
{
  _id = "";
  _label = "";
  _length = 1;
...
WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label)
{
  init(id, label, "", 0, "", WFM_LABEL_BEFORE);
}

WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const String& defaultValue, int length)
{
  init(id, label, defaultValue, length, "", WFM_LABEL_BEFORE);
}

WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const String& defaultValue, int length, const String& custom)
{
  init(id, label, defaultValue, length, custom, WFM_LABEL_BEFORE);
}

WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const String& defaultValue, int length, const String& custom, int labelPlacement)
{
  init(id, label, defaultValue, length, custom, labelPlacement);
}


WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const int defaultValue, int length)
{
  init(id, label, String(defaultValue), length, "", WFM_LABEL_BEFORE);
}

WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const int defaultValue, int length, const String& custom)
{
  init(id, label, String(defaultValue), length, custom, WFM_LABEL_BEFORE);
}

WiFiManagerParameter::WiFiManagerParameter(const String& id, const String& label, const int defaultValue, int length, const String& custom, int labelPlacement)
{
  init(id, label, String(defaultValue), length, custom, labelPlacement);
}
void WiFiManagerParameter::init(const String& id, const String& label, const String& defaultValue, int length, const String& custom, int labelPlacement)
{
  _id = id;
  _label = label;
...
void WiFiManagerParameter::setValue(const String& defaultValue)
{
  if (_id.length() < 2)
  {
...
const String& WiFiManagerParameter::getValue() const
{
  return _value;
}
...
const String& WiFiManagerParameter::getID() const
{
  return _id;
}
const String& WiFiManagerParameter::getPlaceholder() const
{
  return _label;
}
const String& WiFiManagerParameter::getLabel() const
{
  return _label;
}
...
const String& WiFiManagerParameter::getCustomHTML() const
{
  return _customHTML;
}
 ...
 */
bool WiFiManager::addParameter(const std::shared_ptr<WiFiManagerParameter>& p)
{

  // check param id is valid, unless null
  if (p->getID().length() > 1)
  {
    for (size_t i = 0; i < strlen(p->getID().c_str()); i++)
    {
...
String WiFiManager::getParam(const String& name)const
{
  String value;
...
bool WiFiManager::addParameter(WiFiManagerParameter* p)
{
  ...
  if (p->getID().length())
  {
    for (size_t i = 0; i < strlen(p->getID().c_str()); i++)
    {
      if (!(isAlphaNumeric(p->getID().c_str()[i])) && !(p->getID().c_str()[i] == '_'))
      {
  ...
  try
  {
    std::shared_ptr<WiFiManagerParameter> _p = nullptr;
    _p.reset(p);
    _params.push_back(_p);
    _paramsCount++;
 ...
 * @access public
 */
const auto& WiFiManager::getParameters()
{
  return _params;
}
...
bool WiFiManager::setupHostname(bool restart)
{
  ...
  else
  {
    ...
    DEBUG_WM(DEBUG_DEV, F("setupHostname: "), _hostname.c_str());
...
bool WiFiManager::startAP()
{
  ...
  if (ret && _hostname != "")
  {
    ...
    DEBUG_WM(DEBUG_VERBOSE, F("setting softAP Hostname:"), _hostname.c_str());
    if (!res)
...
void WiFiManager::setupDNSD()
{
  ...
  dnsServer->start(DNS_PORT, String("*"), WiFi.softAPIP());
}
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        for (int j = i + 1; j < n; j++)
        {
          if (cssid == WiFi.SSID(indices[j]))
          {
            ...
            DEBUG_WM(DEBUG_VERBOSE, F("DUP AP:"), (WiFi.SSID(indices[j]).c_str()));
...
String WiFiManager::getParamOut()
{
  ...
  if (_paramsCount > 0)
  {
    ...
    for (const auto& param : _params)
    {
      ...
      String pitem;
      switch (param->getLabelPlacement())
      {
      ...
      if (param->getID().length())
      {
        if (tok_I)
          pitem.replace(FPSTR(T_I), (String)FPSTR(S_parampre)); // T_I id number
        if (tok_i)
          pitem.replace(FPSTR(T_i), param->getID().c_str()); // T_i id name
        if (tok_n)
          pitem.replace(FPSTR(T_n), param->getID().c_str()); // T_n id name alias
        if (tok_p)
          pitem.replace(FPSTR(T_p), FPSTR(T_t)); // T_p replace legacy placeholder token
        if (tok_t)
          pitem.replace(FPSTR(T_t), param->getLabel().c_str()); // T_t title/label
        snprintf(valLength, 5, "%d", param->getValueLength());
        if (tok_l)
          pitem.replace(FPSTR(T_l), valLength); // T_l value length
        if (tok_v)
          pitem.replace(FPSTR(T_v), param->getValue().c_str()); // T_v value
        if (tok_c)
          pitem.replace(FPSTR(T_c), param->getCustomHTML().c_str()); // T_c meant for additional attributes, not html, but can stuff
      }
      else
      {
...
const String WiFiManager::GetConfigSSID() const
{
  const String ret = _ssid.c_str();
  return std::move(ret);
}
const String WiFiManager::GetConfigPassword() const
{
  const String ret = _pass.c_str();
  return std::move(ret);
}
/**
...
void WiFiManager::doParamSave()
{
  ...
  if (_paramsCount > 0)
  {
    ...
    for (const auto& param : _params)
    {
      ...
      String name = (String)FPSTR(S_parampre);
      String value;
      ...
      DEBUG_WM(DEBUG_VERBOSE, (String)param->getID().c_str() + ":", value);
...
String WiFiManager::getInfoData(String id)
{
  ...
  else if (id == F("apmac"))
  {
    ...
    p.replace(FPSTR(T_1), WiFi.softAPmacAddress().c_str());
  }
#ifdef ESP32
  else if (id == F("aphost"))
  {
  ...
  else if (id == F("apbssid"))
  {
    ...
    p.replace(FPSTR(T_1), WiFi.BSSIDstr().c_str());
  }
  ...
  else if (id == F("stassid"))
  {
    ...
    p.replace(FPSTR(T_1), htmlEntities((String)(WiFi_SSID().c_str())));
  }
  else if (id == F("staip"))
  {
  ...
  else if (id == F("stamac"))
  {
    ...
    p.replace(FPSTR(T_1), (WiFi.macAddress().c_str()));
  }
  else if (id == F("conx"))
  {
 ...
 * @param char element
 */
void WiFiManager::setCustomHeadElement(const String& element)
{
 ...
 */
void WiFiManager::setMenu(const String menu[], uint8_t size)
{
 ...
 */
void WiFiManager::setMenu(const std::vector<String>& menu)
{
