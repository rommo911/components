...
void disp_spi_add_device_config(spi_host_device_t host, spi_device_interface_config_t *devcfg)
{
    ...
    esp_err_t __attribute__((unused)) ret=spi_bus_add_device(host, devcfg, &spi);
    assert(ret==ESP_OK);
}
...
void disp_spi_remove_device()
{
    ...
    esp_err_t __attribute__((unused)) ret=spi_bus_remove_device(spi);
    assert(ret==ESP_OK);
}
...
{
    esp_err_t __attribute__((unused)) ret = spi_device_acquire_bus(spi, portMAX_DELAY);
    assert(ret == ESP_OK);
}
