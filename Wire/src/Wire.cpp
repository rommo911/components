...
TwoWire::TwoWire(uint8_t bus_num)
    : num(bus_num & 1), sda(-1), scl(-1), rxIndex(0), rxLength(0), txLength(0), txAddress(0), _timeOutMillis(50), nonStop(false)
#if !CONFIG_DISABLE_HAL_LOCKS
    ,
    nonStopTask(NULL), lock(NULL)
#endif
    ,
...
bool TwoWire::initPins(int sdaPin, int sclPin)
{
    ...
    { // default param passed
        if (num == 0)
        {
            if (sda == -1)
            {
                ESP_LOGE(TAG, "no Default SDA Pin for Second Peripheral");
    ...
    { // default param passed
        if (num == 0)
        {
            if (scl == -1)
            {
                ESP_LOGE(TAG, "no Default SCL Pin for Second Peripheral");
                return false; // no Default pin for Second Peripheral
            }
            else
            {
...
bool TwoWire::setPins(int sdaPin, int sclPin)
{
    ...
    // acquire lock
    if (xSemaphoreTake(lock, pdMS_TO_TICKS(_timeOutMillis)) != pdTRUE)
    {
    ...
    {
        ESP_LOGE(TAG, "bus already initialized. change pins only when not.");
    }
#if !CONFIG_DISABLE_HAL_LOCKS
    // release lock
    xSemaphoreGive(lock);
...
bool TwoWire::begin(uint8_t addr, int sdaPin, int sclPin, uint32_t frequency)
{
    ...
    if (is_slave)
    {
        ESP_LOGW(TAG, "Bus already started in Slave Mode.");
        started = true;
        goto end;
    }
    if (i2cIsInit(num))
    {
        ESP_LOGE(TAG, "Bus already started in Master Mode.");
        goto end;
    }
...
bool TwoWire::begin(int sdaPin, int sclPin, uint32_t frequency)
{
    ...
    if (is_slave)
    {
        ESP_LOGE(TAG, "Bus already started in Slave Mode.");
        goto end;
    }
    if (i2cIsInit(num))
    {
...
bool TwoWire::end()
{
    ...
    if (lock != NULL)
    {
        ...
        if (is_slave)
        {
            ...
            {
                ESP_LOGE(TAG, "i2cSlaveDeinit err= %s", esp_err_to_name(err));
            }
        }
        else if (i2cIsInit(num))
        {
            ...
            if (err != ESP_OK)
            {
                ESP_LOGE(TAG, "i2cDeinit err= %s", esp_err_to_name(err));
            }
...
uint32_t TwoWire::getClock()
{
...
#if !CONFIG_DISABLE_HAL_LOCKS
    // acquire lock
    if (lock == NULL || xSemaphoreTake(lock, pdMS_TO_TICKS(_timeOutMillis)) != pdTRUE)
    {
        ESP_LOGE(TAG, "could not acquire lock");
    }
    else
    {
        ...
        if (is_slave)
        {
            ESP_LOGE(TAG, "Bus is in Slave Mode");
        }
        else
        {
...
uint8_t TwoWire::endTransmission(bool sendStop)
{
    ...
    if (sendStop)
    {
        //ESP_LOGI(TAG, "i2cWrite add=%x", txAddress);
        err = i2cWrite(num, txAddress, txBuffer, txLength, _timeOutMillis);
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "i2cWrite add=%x err= %s", txAddress, esp_err_to_name(err));
        }
#if !CONFIG_DISABLE_HAL_LOCKS
        // release lock
        xSemaphoreGive(lock);
...
uint8_t TwoWire::requestFrom(uint16_t address, uint8_t size, bool sendStop)
{
        ...
        )
    {
        ...
        {
            ESP_LOGE(TAG, "Unfinished Repeated Start transaction! Expected address do not match! %u != %u", address, txAddress);
            return 0;
        }
        nonStop = false;
        rxIndex = 0;
        ...
        //ESP_LOGI(TAG, "i2cWriteReadNonStop ");
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "i2cWriteReadNonStop Error! err= %s", esp_err_to_name(err));
            return 0;
        }
    }
    else
    {
        ...
        if (err != ESP_OK)
        {
            ESP_LOGE(TAG, "i2cRead Error! err= %s", esp_err_to_name(err));
        }
    }
#if !CONFIG_DISABLE_HAL_LOCKS
    // release lock
